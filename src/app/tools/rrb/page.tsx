...
const TOOL_CONFIG = {
  ...
  colorBg: "bg-brand-pink/10",
  colorBorder: "border-brand-pink/30",
  width: 140,
  height: 60,
  minKb: 30,
  maxKb: 49,
  ...
  conditions: [
    ...
    "File size: 30 to 49 KB",
    "Minimum dimension: 140 px (W) × 60 px (H)",
    "Scan resolution: minimum 100 DPI",
    "Signature must be written by candidate only",
    "Use black ink pen on white paper",
    "Signature must be in running handwriting (cursive/joined)",
    "NOT in BLOCK, CAPITAL, or DISJOINTED letters",
    "Must be clear, legible, and entirely visible",
  ],
};
...
export default function RRBToolPage() {
  ...
  async function onAutoFix() {
    ...
    try {
      ...
      const outputFile = new File([blob], `${file.name.split(".")[0]}-rrb.jpg`, { type: TOOL_CONFIG.format });
  ...
  return (
    <main className="min-h-screen">
      ...
      <section className="relative overflow-hidden pt-20 pb-16 md:pt-32 md:pb-24">
        <div className="absolute inset-0 -z-10 bg-gradient-to-br from-brand-pink/15 via-brand-pink/5 to-transparent" />
        ...
        <div className="mx-auto max-w-7xl px-4 md:px-6">
          ...
          <div className="space-y-6">
            ...
            <div className="grid md:grid-cols-3 gap-4 pt-4">
              ...
              <div className={`p-4 rounded-xl border ${TOOL_CONFIG.colorBorder} ${TOOL_CONFIG.colorBg}`}>
                <p className="text-xs text-foreground/60 uppercase tracking-wider mb-1">Dimensions (Min)</p>
                <p className={`text-xl font-bold ${TOOL_CONFIG.colorText}`}>{TOOL_CONFIG.width}×{TOOL_CONFIG.height} px</p>
              </div>
              <div className={`p-4 rounded-xl border ${TOOL_CONFIG.colorBorder} ${TOOL_CONFIG.colorBg}`}>
                <p className="text-xs text-foreground/60 uppercase tracking-wider mb-1">Resolution</p>
                <p className={`text-xl font-bold ${TOOL_CONFIG.colorText}`}>100 DPI min</p>
              </div>
            </div>
          </div>
        </div>
      ...
      <section className="py-12 md:py-16">
        <div className="mx-auto max-w-4xl px-4 md:px-6">
          <div className="ui-modal-shell overflow-hidden">
            <span className="ui-ambient-orb right-5 top-5 h-20 w-20 bg-brand-pink/30" />
      ...
      <SuccessPopup
        ...
        message="Your RRB signature has been formatted and quality-balanced for upload readiness."
        onClose={() => setShowSuccess(false)}
        onDownload={onDownloadProcessed}
        downloadLabel="Download RRB File"
      />
    </main>
  );
}
