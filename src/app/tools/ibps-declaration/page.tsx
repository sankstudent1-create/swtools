...
const TOOL_CONFIG = {
  ...
  org: "IBPS PO / Clerk / SO Upload Module",
  icon: "📝",
  ...
  conditions: [
    ...
    "File size: 50 to 100 KB",
    "Dimensions: 800 px × 400 px",
    "Aspect ratio: 2:1 landscape",
    "Declaration text should be handwritten clearly",
    "Use black ink on white plain paper",
    "No cut edges or folding shadows",
    "Entire declaration line must be visible",
  ],
};
...
export default function IBPSDeclarationPage() {
  ...
  async function onAutoFix() {
    ...
    try {
      ...
      const outputFile = new File([blob], `${file.name.split(".")[0]}-ibps-declaration.jpg`, { type: TOOL_CONFIG.format });
  ...
  return (
    <main className="min-h-screen">
      <section className="relative overflow-hidden pt-20 pb-16 md:pt-32 md:pb-24">
        <div className="absolute inset-0 -z-10 bg-gradient-to-br from-emerald-400/15 via-brand-sky/5 to-transparent" />
          ...
          <div className="space-y-6">
            ...
            <div className="grid md:grid-cols-3 gap-4 pt-4">
              ...
              <div className={`p-4 rounded-xl border ${TOOL_CONFIG.colorBorder} ${TOOL_CONFIG.colorBg}`}>
                ...
                <p className={`text-xl font-bold ${TOOL_CONFIG.colorText}`}>2:1 Landscape</p>
              </div>
            </div>
          </div>
        </div>
      ...
      <section className="py-12 md:py-16">
        <div className="mx-auto max-w-4xl px-4 md:px-6">
          <div className="ui-modal-shell overflow-hidden">
            <span className="ui-ambient-orb right-5 top-5 h-20 w-20 bg-emerald-400/30" />
            <span className="ui-ambient-orb bottom-6 left-4 h-16 w-16 bg-brand-sky/20" />
              ...
              <div className="relative z-10 space-y-6">
                <div className="space-y-3">
                  ...
                  <div className="flex items-center gap-4">
                    ...
                    <h2 className="font-heading text-2xl font-bold">Upload Declaration Image</h2>
                  </div>
                </div>
                ...
                <label className="block">
                  ...
                  >
                    ...
                    <p className="text-xs text-foreground/60">PNG, JPG up to 10MB • Auto crop, resize and compress</p>
                    <input type="file" accept="image/*" onChange={onFileChange} className="hidden" />
                  </div>
                </label>

                {error && <div className="rounded-lg border border-red-500/40 bg-red-500/10 p-4 text-sm text-red-500">{error}</div>}
                ...
                <div className="grid gap-6 md:grid-cols-2">
                  <div className="ui-preview-card p-4">
                    ...
                    {imagePreviewUrl ? (
                      ...
                      <div className="flex h-64 items-center justify-center rounded-lg border border-dashed border-white/15 bg-black/20 text-foreground/55">No image selected</div>
                    )}
                  </div>
                  ...
                  <div className="ui-preview-card p-4">
                    ...
                    {processedFileUrl ? (
                      <>
                        ...
                        <a href={processedFileUrl} download={processedFileName} className={`block w-full rounded-lg bg-gradient-to-r ${TOOL_CONFIG.color} px-4 py-2 text-center font-semibold text-black transition hover:shadow-lg`}>
                          Download {processedFileName}
                        </a>
                      </>
                    ) : (
                      <div className="flex h-64 items-center justify-center rounded-lg border border-dashed border-white/15 bg-black/20 text-foreground/55">Process image to see output</div>
                    )}
                  </div>
                </div>

                <button onClick={onAutoFix} disabled={!file || isProcessing} className={`w-full rounded-xl bg-gradient-to-r ${TOOL_CONFIG.color} px-6 py-3 font-semibold text-black transition hover:shadow-lg disabled:opacity-50`}>
                ...
                {validation && (
                  <div className={`ui-preview-card p-4 ${isValid ? "border-green-500/30 bg-green-500/10" : "border-amber-500/30 bg-amber-500/10"}`}>
                    <div className="mb-3 flex items-center gap-2">
                      ...
                      <span className={`font-semibold ${isValid ? "text-green-500" : "text-amber-500"}`}>{isValid ? "✓ Valid" : "Needs Fix"}</span>
                    </div>
      ...
      <SuccessPopup
        ...
        title="Declaration Processed"
        message="Your IBPS declaration image is now normalized for size, aspect ratio and portal upload compliance."
        onClose={() => setShowSuccess(false)}
        onDownload={onDownloadProcessed}
        downloadLabel="Download Declaration"
      />
    </main>
  );
}
