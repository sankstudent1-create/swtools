...
export default function TDCommissionPage() {
  ...
  const handlePreview = async () => {
    ...
    setIsPdfBusy(true)
    try {
      const res = await fetch('/api/tools/td-commission/preview', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ office, rows }),
      })

      if (res.status === 401) {
        window.location.href = `/auth/login?next=${encodeURIComponent('/tools/td-commission')}`
        return
      }
      if (!res.ok) {
  ...
  const handleDownload = async () => {
    ...
    setIsPdfBusy(true)
    try {
      const res = await fetch('/api/tools/td-commission/download', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ office, rows }),
      })
    ...
    <div className="min-h-screen bg-[#050505] text-white selection:bg-emerald-500/30 relative overflow-x-hidden font-sans">
      ...
      <div className="fixed bottom-0 left-0 right-0 z-50 flex gap-3 justify-end flex-wrap px-6 py-4 border-t border-white/10 bg-[#050505]/80 backdrop-blur-xl">
        ...
        <button onClick={handleDownload}
          disabled={isPdfBusy}
          className="flex items-center gap-2 px-6 py-2.5 rounded-xl bg-emerald-500 hover:bg-emerald-400 text-black font-semibold text-[13.5px] transition-all hover:shadow-[0_0_20px_rgba(16,185,129,0.3)] hover:-translate-y-0.5 disabled:opacity-50 disabled:hover:shadow-none disabled:hover:translate-y-0">
