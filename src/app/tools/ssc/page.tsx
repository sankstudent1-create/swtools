...
const TOOL_CONFIG = {
  ...
  colorBg: "bg-brand-orange/10",
  colorBorder: "border-brand-orange/30",
  width: 472,
  height: 236,
  minKb: 10,
  maxKb: 20,
  ...
  conditions: [
    ...
    "Dimension: ~4.0 cm (W) × 2.0 cm (H)",
    "Pixel default: ~472 × 236 px (300 DPI)",
    "Signature must be clear and legible",
    "Blurred signatures will be rejected",
    "For PwD (VH) candidates, thumb impression is allowed",
  ],
};
...
export default function SSCToolPage() {
  ...
  async function onAutoFix() {
    ...
    try {
      ...
      const outputFile = new File([blob], `${file.name.split(".")[0]}-ssc.jpg`, { type: TOOL_CONFIG.format });
  ...
  return (
    <main className="min-h-screen">
      ...
      <section className="relative overflow-hidden pt-20 pb-16 md:pt-32 md:pb-24">
        <div className="absolute inset-0 -z-10 bg-gradient-to-br from-brand-orange/15 via-brand-orange/5 to-transparent" />
          ...
          <div className="space-y-6">
            ...
            <div className="grid md:grid-cols-3 gap-4 pt-4">
              ...
              <div className={`p-4 rounded-xl border ${TOOL_CONFIG.colorBorder} ${TOOL_CONFIG.colorBg}`}>
                ...
                <p className={`text-xl font-bold ${TOOL_CONFIG.colorText}`}>4.0:2.0</p>
              </div>
            </div>
          </div>
        </div>
      ...
      <section className="py-12 md:py-16">
        <div className="mx-auto max-w-4xl px-4 md:px-6">
          <div className="ui-modal-shell overflow-hidden">
            <span className="ui-ambient-orb right-5 top-5 h-20 w-20 bg-brand-orange/30" />
      ...
      <SuccessPopup
        ...
        message="Your SSC signature is now optimized with modal-quality validation and ready to download."
        onClose={() => setShowSuccess(false)}
        onDownload={onDownloadProcessed}
        downloadLabel="Download SSC File"
      />
    </main>
  );
}
