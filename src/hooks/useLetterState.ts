...
const INITIAL_STATE: AppState = {
  ...
  logoL: null,
  logoR: null,
  posL: { ...DEFAULT_LOGO_POS, x: 42, y: 18, w: 100 },
  posR: { ...DEFAULT_LOGO_POS, x: 650, y: 18, w: 100 },
  sigUrl: null,
  sigMode: 'draw',
  showEncl: false,
  showCopy: false,
...
export function useLetterState() {
  ...
  const fillFromAI = useCallback((data: AILetterData, isFull: boolean = false, model?: string) => {
    ...
    setState(s => {
      ...
      logoL: newLogoL,
      logoR: newLogoR,
      posL: isHigherPost ? { ...s.posL, x: 347, y: 18, w: 100, placed: true } : (isFull ? { ...s.posL, x: 42, y: 18, w: 100, placed: true } : s.posL),
      posR: (isFull && newLogoR) ? { ...s.posR, x: 650, y: 18, w: 100, placed: true } : s.posR,
